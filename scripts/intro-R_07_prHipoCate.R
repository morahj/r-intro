...
# Al tomar una muestra aleatoria de 15 pacientes de un tratamiento se encontró que la reducción de peso tuvo en adultos un promedio de 5kg con una desviación de 0,8Kg.
# a) Construya el intervalo de confianza del 95% para la desviación estándar de la reducción de peso con el tratamiento.

# a) Construcicón del Intervalo de confianza del 99% para la desviación

# Valores de chi-cuadrado para el intervalo
e1b_chi_izq <- qchisq(.025, (14)) # Chi-cuadrado para la cola izquierda.
e1b_chi_der <- qchisq(.975, (14)) # Chi-cuadrado para la cola derecha.

# Límites del intervalo de confianza
e1b_lim_sup <- sqrt( (14 * (.8) ^ 2 / e1b_chi_izq)) # Límite superior.
e1b_lim_inf <- sqrt( (14 * (.8) ^ 2 / e1b_chi_der)) # Límite inferior.

e1b_ic <- c(e1b_lim_inf, e1b_lim_sup) # Asignación de límites del IC a una variable.
e1b_ic # Intervalo de confianza de la varianza.

rm(e1b_chi_izq, e1b_chi_der, e1b_lim_sup, e1b_lim_inf, e1b_ic) # Borra los objetos creados para el ejercicio.
...
e2_chi_null <- qchisq(.99, 9) # Valor de chi-cuadrado para la zona crítica.
e2_chi <- (9 * (10.2) ^ 2 / (8.5) ^ 2 ) # Valor de chi-cuadrado de prueba usando la fórmula.

e2_chi_null # Chi-cuadrado de zona crítica.
e2_chi # Chi-cuadrado de prueba.

rm(e2_chi_null, e2_chi) # Borra los objetos creados para el ejercicio.
...
e3_tabla_obs <- matrix(c(250, 170, 50, 30), ncol = 4) # Tabla de valores observados.
colnames(e3_tabla_obs) = c("Bogota", "Medellin", "Cali", "Otra") # Nombre para las columnas de la tabla.
e3_tabla_obs # Tabla de valores observados.

e3_tabla_esp <- c(.4, .3, .2, .1) # Valores esperados según enunciado del ejercicio.

# Estadísticos chi-cuadrado
e3_chi_null <- qchisq(.975, 3) # Chi-cuadrado de zona crítica.

# Prueba de chi-cuadrado para bondan de ajuste entre "tabla3" y "tabla3_esperados".
e3_chi <- chisq.test(e3_tabla_obs, p = e3_tabla_esp) 

e3_chi # Chi-cuadrado de prueba.
e3_chi_null # Chi-cuadrado de zona crítica.


rm(e3_tabla_obs, e3_tabla_esp, e3_chi_null, e3_chi) # Borra los objetos creados para el ejercicio.
...
e4_tabla # Tabla completa.

# Estadísticos chi-cuadrado 
e4_chi_null <- qchisq(.95, 2) # Valor chi-cuadrado de zona crítica.

# Prueba de chi-cuadrado de "tabla4"
e4_chi <- chisq.test(e4_tabla) # Chi-cuadrado de prueba.

e4_chi # Chi-cuadrado de prueba.
e4_chi_null # Chi-cuadrado de zona crítica.


rm(e4_tabla, e4_chi_null, e4_chi) # Borra los objetos creados para el ejercicio.